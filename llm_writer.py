...
1. **종결어미 (모든 섹션 동일 적용 — VI. Executive Summary 포함)**: "~임", "~하였음", "~요인임", "~확인함", "~의미를 둠", "~판단됨" 등 실무 보고서 투 사용. ⚠️ **VI 섹션에서도 절대 "~니다/~습니다" 사용 금지**. 입력 인사이트가 "~니다"로 끝나면 모두 "~임/~함"으로 변환: "높았습니다"→"높음", "기록했습니다"→"기록함", "필요합니다"→"필요함", "이루어졌습니다"→"이루어짐", "달성했습니다"→"달성함". "~이다", "~분석된다" 같은 학술적 종결도 금지
...
4. **팩트 중심 간결체 — 신문 기사 톤**: 사실 전달에 집중. 한 문장에 하나의 팩트. 드라마틱한 강조어 금지 ("매우", "획기적인", "인상적인", "두드러진", "기념비적인" 등). 미술관 보고서는 비평·홍보가 아닌 **사실의 기록**임을 명심.
5. **원인-결과 연결**: "~로 인한", "~가 요인임" 등 인과 관계를 명시적으로 서술
6. **정량 + 정성 (사실 첨언, 평가 금지)**: 수치를 제시한 뒤 그것이 가리키는 바를 사실 기반으로 한 문장 첨언. 과잉 해석 금지. **평가어 사용 금지** ("성공적", "기대 미달", "탁월한" 등). 단정 대신 한정사 사용("~로 확인됨", "~에 해당함", "~로 산출됨", "~로 보임"). 판단은 디렉터의 영역.
7. **한 섹션당 2~5문장**: 간결하게. 각 문장은 구체적 수치나 사실을 포함

8. **어휘 순화 — 최종 결재권자가 이해 가능한 표현 사용** (⚠️ 보고서는 이사장·관장 등 외부 결재자가 읽음):
   - "N개 전시 중 M위" 같은 내부 통계 표현 금지 → "기존 전시 중 M위" 또는 "역대 전시 중 M위"로 추상화
   - "1유형", "2유형" 같은 내부 분류 코드 절대 사용 금지. 입력에 "기존 기획전", "기존 특별전", "기존 전시", "역대 전시"로 자연어 표기되어 있으므로 그대로 유지
   - 비교 대상이 명확하지 않은 수치(예: 표본 크기)는 노출하지 말 것

9. **VI. Executive Summary 섹션의 특별 성격** (⚠️ 이 섹션은 다른 섹션과 다릅니다):
   - VI는 **평가(verdict) 섹션이 아니라 중립적 종합(documentation) 섹션**. 디렉터가 자신의 판단을 내릴 수 있도록 사실을 정렬·종합하는 그릇임
   - 본문 II~V장의 핵심 사실을 신문 기사처럼 중립적으로 종합. 결론을 단정하지 말 것
   - "성공적이었음", "기대를 충족함", "탁월한 성과" 등 평가 표현 금지. 대신 사실 그 자체를 진술: "관객 수가 역대 전시 평균을 X% 상회함", "예산 회수율 Y%로 산출됨"
   - 미술관 보고서는 판정이 아닌 기록. 디렉터의 가치 판단 영역은 침범하지 않을 것
   - VI 섹션의 분량은 본문 다른 섹션과 동일(2~5문장). 본문의 사실을 압축·재배치할 뿐, 새로운 평가를 만들지 않음
...
- 사실 종합 시 "~에 의미를 둠", "~으로 확인됨", "~에 해당함" 등 절제되고 중립적인 어조 유지. "긍정적/부정적"으로 명시적 평가하지 말 것
